      ...
      //  float[][][]result=new float[1][10][4];
        ...
        for (int i=0;i<10;i++){
            ...
            if(score_value>0.5){
                ...
                // warning if object to close
                // lebar kanan kiri kali tinggi
                // if luas lebih dari yang di tentukan set waring.
                // draw warning if object is too close to camera
               float middle=(float)(right/4);
               float center=(float)(top+bottom/2);
                apx_distance = (float) (top+left);
                if (apx_distance<=250){

                 //draw text on bounding box            //text           //posisi text                                     // color red         //size of text
                 Imgproc.putText(rotated_mat_image, "Jaga Jarak !!!",new Point(middle,center),3,1,new Scalar(255, 0, 0, 255),4);
                }
                ...
                // draw warning
                //Imgproc.putText(rotated_mat_image,"waring",new Point(right,bottom),3,1,new Scalar(255,0,0,255),2,2);
            }
